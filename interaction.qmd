...
The below sections will focus on data science' most popular editors, namely Visual Studio Code and RStudio. Hence I would like to mention at least some IDE juggernaughts here, for the reader looking or alternatives: [Eclipse](https://www.eclipse.org/) (mostly Java but tons of plugins for other languages), or JetBrains' [IntelliJ](https://www.jetbrains.com/idea/) (Java) and [PyCharm](https://www.jetbrains.com/pycharm/) (Python).
...
- cmd+shift+option+R, while cursor is within a function's body (create a roxygen documentation skeleton)
...
Outside the R universe (and to some degree even inside it), Visual Studio Code became to the go-to editor in data science. 
Microsoft's Visual Studio Code started out as modular, general purpose IDE not focused on a single language.
In the meantime there is not only great Python support, but also auto-completion, code highlighting for R or Julia and many other languages. 
...
Notebooks are yet another popular choice for a homecourt among data scientists and analysts. 
...
The basic idea of notebooks is to run webserver in the background to present a browser based frontend to the developer.
...
<!--
# Python code to draw a very simple NBA court, 

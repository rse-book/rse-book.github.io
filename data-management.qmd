...
In addition normative restrictions that come with handling data, the options and choices of data dissemination are a realm of its own. 
Just like software publications you should not just 'drop' data without a license and instructions on accetable use of the data. 


### Etiquette

Just because content is publicly avaiable on a website, it does not automatically mean that bulk downloads, aggregation and re-publishing are ok.
...
In other words, the new type of researcher discussed in this book needs to be aware of potential legal and social consequences. The {polite} R package quoted above is an example for an alternative approach that favors etiqutte over hiding IP addresses to avoid access denial. 


### Privacy 

Privacy in data science is a complex issue and could legitamtely fill a book on its own.
Though I cannot comprehensively cover privacy in a section, it is important to me to raise awareness and hopefully create an entrypoint to the matter. 
...
A single extra ordinary feature or an exotic combination of features can identify an individual within a dataset or at least expose a group. 
This is why merging merging multiple dataset may also cause privacy concerns when datasets where not created to be merged in the first place and/or individuals were not aware that merging was possible. 
...
Most universities and larger corpersations will have an officer to run these things by. 
...
It does not have to be you who has been trusted to work with data that qualify as senstive. 
Someone on your team or within your network might have been and that person may trust you more than a stranger and be less vigilant when harmful actions are executed your name. 
...
Use a password manager so the passwords do not need to be your cats name for you to remember. 
Use a password manager so you can afford to not have the same passwords for multiple applications. 
...
In addition to above brainy smurf advice, let me mention security as a reason to consider using a database to manage and archive your data for the long haul. 
Enterprise level databases allow for granular access management and help to stay ahead of users and their rights with respect to your database's entries. 


### Open Data 

Open Data refers to publicly available data that are available free of license costs and in machine readable fashion. 
...
pendata initiatives of your country as well as major international organizations will help you to create interoperable datasets.
In addition, open data organizations provide you with a publication channel for your data or with data catalog that stores the data description. 
...
Their XML based format has become an international standards which lead to the implementation of SDMX read and write routines in statistical software.
Whether you think about the conditions of your own data publications or about a source for own research project, make sure to consider open data. 

...
The KOF Swiss Economic Institute at ETH Zurich provides such a wrapper in the {kofdata} R package [@kofdata]. The underlying API allows access to the KOF time series archive database and obtain data and meta information alike. The below code snippet gets data from the API and uses another KOF built library ({tstools} [@tstools]) to visualize the returned time series. 

...
Wonder what's really on the image? Umbrella or not :) ? Try to reproduce the this example or come up with your own wrapper from scratch ... 


## Create Your Own API

The ability to expose data is a go-to skill in order to make research reproducible and credible. Especially when data get complex and require thorough description in order to remain reproducible for others, a programmtic, machine readable approach is the way to go. 
...
Hence both the on the on disk representation as well as the in-memory representation are equally well suited for nested data. 
...
According to [Mozilla's defintion](https://developer.mozilla.org/en-US/docs/Web/HTTP/Methods), the GET method "requests a representation of the specified resource.
...
Let's assume you have a working [nodejs](https://nodejs.org/en/) Javascript runtime environment installed on your local development machine, so you can run javascript files outside of a web browser. 
Such a setup mimicks the situation on webserver with nodejs installed. 
...
First, create a folder `api`, go to freshly created directory and initiate a node package. 
...
just sleep walk through the interactive dialog accepting all defaults. This created a package.json file to keep track of dependencies and their package versions used in your protect. Once done, add express using the npm package manager. 

```
npm install express
```

Now, that we installed the javascript framfework expressjs, we can use the framework to create an minimal web application that serves an API endpoint using the node runtime environment. 
Consider a minimal hello-world example that does about the same as static file example of the previous action: 
...
We simply added a mount point using `app.use` (instead of `app.get`) which makes sure than everything past `/static` is affected by the logic added to this mount. 
...
Though this is only one, admittedly made up example of mapping URLs and parameters to functions via HTTP(S), it hints at possibilities of dynamic APIs from database queries to web forms and many other applications. 
The above example shows also how frameworks like [express.io]() or [plumber]() facilitate the definition of machine-to-machine interfaces even for less experienced developers. 
...
This development has lead to a wide array of offerings that allow us to run our code in a language agnostic environment. 
In addition to the major cloudplatforms, git providers that offer a CI/CD toolchain are a good option to run our code on a remote container. 
...
The script uses {knitr} to render an RMarkdown document to a markdown document that is automatically rendered to pretty HTML file when pushed to a repository hosted on GitHub.
...
Embedded in a bit of descriptive text before and after the code, the main code chunk of the .Rmd template file is equally straight forward: 
...
Last but not least we print the system date of the runtime environment -- in our case the GitHub Actions docker container. 
Running the the README.**Rmd** file yields two artifacts: a README.**md** file and a .png graph located in a `figure/` folder. 
...
When located properly according to GitHub's convention, i.e., in a hidden `github/worflows` folder, GitHub identifies the .yanl file as input and allows to execute the worflow.
...
First of all, the a below file gives our workflow a name to identify the workflow among other workflows defined within the same repository. 
...
Notice that `actions/checkout@v3` is different from the other steps because it is taken from GitHub's market place for actions.
Instead of writing standard operations, namely checking out the git repository we're working with to the container that runs the action, from scratch, we use the market place action for that. 
Be aware though that there is tradeoff between convenience and transparency. 
When I was teaching this year and wanted to recycle an example from the previous year it was not all that convenient. 
Only one year later my example that leaned heavily on market place actions was not working anymore. 
...
This file is going to be replaced anyway and we want to avoid any rights conflicts overwriting the file. 
...
In combination with the *shebang* comment inside the file, we can directly execute the .R file with the `./` prefix because it know which executable to use to run it. 
...
After the process is done, the container will stopped and removed. 
...
![Maybe I am late, but I look goood. Johny Bravo representing the frontend](images/johnny.png)
<!--
...
Also, consider that processes can be turned parallel at different levels: you could simply use your local machine's cores and a parellization implementation such as R's {future} or {parallel} to parrelize locally. 
...
Of course, if you have access to a HPC cluster, this may not be a disadvantage at all (depending how your cluster balances load and manages access to resources). 
...
The following R example uses the {microbenchmark} package to check the effect parallel computing on running seasonal adjustment of multiple time series.
...
The first statement performs 100 adjustments sequentially, the second statement uses parallel computing to spread computations across my machines multiple processors. 
...
Yet, the key take away from the above exercise is not the parallel computation itself, but the ability to the need to go parallel and how. 
Benchmarking does not only help to get a ballpark estimate of the costs should you go outside for more computational power, it a gives you an idea whether the gains from a parallel approach are worth the hassle. 
...
Development time, particularly for newcomers, is very very likely higher than a few minutes. 
Plus, going parallel jeopardizes your cross-platform compatbility depending on the parallel implementation you use.
...
```{r, messages=FALSE, warnigs=FALSE}
library(ggplot2)
library(viridis)
vizframe <- d[,c(1,5)]
...
Even though I cannot really aim at a comprehensive section on the *Do's and Dont's*, I would like to share a few common sense practices here.
...
All these scripting language *can* keep track of the exact library versions used in a project. 

```{bash, eval=FALSE}
pip freeze | grep -v "pkg-resources" > requirements.txt
```

The above pip command extracts dependencies from a python projects folder and write it to a requirements file. 
...
And even though it is common sense in engineering, it took data science, R in particular quite a while to really establish going the extra mile and keep track of a projects dependencies. 
...
After a few more projects, we decide upgrade R to ne next minor release, e.g., go from 4.1. to 4.2. which cause re-installing all the extensions packages. 
...
I see more than one thing that has you set of for trouble before you even started. 
Please

- resist that visceral urge to define the project root folder somewhere at the beginning of yoour code, because your collaborators usually do not have a directory on their machine that bears your name. 
That might not even use the same operating system. 
And before you ask, simply putting the a neutrally named folder on shared NAS storage or service such as dropbox is not really better. 
Doing 
...
The same holds for umlauts and other hieroglyphical characters. Simple use lower case adn either kebap-case or snake_case. 

- work with projects of your favorite IDE. 
The idea of projects is to make a folder the document root 



## Take Some Time Learn How to Play the 'Piano' 

No need to become vim virtuoso (that's essentially Lang Lang status on a computer keyboard), but make sure to learn some basic shortcuts of your environment that help you avoid touching the mouse or trackpad. 




If you are interested in a more throrough R specific collection take a look at Jenny Bryan's and Jim Hester's wonderful *What They Forget to Tell Us About R* book. 

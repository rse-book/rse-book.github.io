...
download_met_images_by_id <- function(ids,
  ...
  message(
    sprintf("
    The following images ids were not public domain and could not be downloaded:\n %s",
            paste(ids[!public], collapse = ",")
  ))
  
  message(
    sprintf("
    The following images ids were public domain and could be downloaded to\n %s: %s",
...
download_swissdata <- function(dataset){
  ...
  gh_url <- paste0("https://raw.githubusercontent.com/",
  "swissdata/demo/master/data")
...
Let's assume you have a working [odejs^[https://nodejs.org/en/] JavaScript runtime environment installed on your local development machine, so you can run JavaScript files outside a web browser. 
...
app.listen(port, () => {
  console.log(`DevOps carpentry demo app 
...
app.listen(port, () => {
  console.log(`DevOps carpentry demo api
   listening on port ${port}`)
})
...
table_list <- url %>%
  read_html() %>%
  html_nodes(xpath = '
  //html/body/div[6]/section/div/section/div[2]/div/div[3]/div/div/div/div/div/div/div/table') %>%
  # turn the HTML table into an R data.frame
  html_table()
# because the above result may potentially contain
# multiple tables, we just use 
# the first table. We know from visua
...
![screenshot of an interactive choropleth, for the actual interactive visit https://devops-carpentry.github.io/book/case-studies.html#sec-map](images/case-choropleth.png)
...
gg + 
  ...
  theme_minimal() +
  ggtitle("
  Comparison of Computation Time (shorter bar is faster)"
  )

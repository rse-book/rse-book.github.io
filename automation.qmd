...
Essentially, a DAG depictures a process that has -- unlike a cycle -- a clear start and ending.
DAGs can be one dimensional and fully seqeuential, but can also run tasks in parallel. 
...
Such operators also allow to execute tasks on remote machine or clusters, not solely the machine or container that serves Airflow itself.

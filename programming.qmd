...
One important word of advice for novice package developers is to think about your dependencies. Do not take dependencies lightly. Of course, it is intriguing to stand on the shoulders of giants. Isn't R great because of its almost 20K extension packages? Isn't exactly this was made R such as popular language? 
...
Hadley's book R packages has a more thorough introduction to testing in R^[https://r-pkgs.org/tests.html] with the testthat [@testthat] package. Though the book is focused on R its introduction to formal testing is very illustrative for anyone interested to add testing to their development routine. 

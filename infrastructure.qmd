...
One of the beautiful things about containers is that due their isolated nature, one can go along way trying out things as containers get destroyed and recreated all the time. 
...
The rocker project designs useful images around the R language ecosystem, builds them on regularly basis and makes them available via dockerhub.
Here, our image allows to run the open source version of shiny server in a docker container. 
...
Online tools either offered as a service or self-hosted can build images serverside. Yet the easiest way to get started with containers is to run and build them locally with Docker Desktop.  
...
Docker Desktop is an easy to use application available on Windwos and OSX. 
...
Modern application often follow modular architecture patterns, i.e., they have a front end, some middle layer such as a REST API and a database backend. 
...
This is where docker could help to create development environment locally that essentially mimicks the production setup and therefore facilitates deployment to production. 

Docker compose allows to define how multiple containers play together. 
Consider the following example file that creates two containers: a shiny webserver and database which can be queried by the shiny server. 
...
Note how images are build from local directories `postgres` and `shiny` that contain Dockerfiles. 
...
## So Why Again Should a Practioner Care? 

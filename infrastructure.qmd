...
![Schematic illustration of a simple server setup for web-based IDE and web application server. The setup would work well with virtual machines as well as with docker\index{Docker} containers. (Source: own illustration.)](images/server-setup3.jpg)
...
While the above *Building Blocks* section contextualizes the container\index{containerization\index{containerization}} approaches, this section gives a simple 101 into the basics of containerization\index{containerization}, enabling the readers to take their first steps in the container world.
...
*DOCKERFILE\index{DOCKERFILE}*s\index{Docker} are text file recipes to create images, i.e., blueprints for containers. One great thing about container images is that they are layered.
